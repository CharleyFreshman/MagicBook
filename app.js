...
function setupEventListeners() {
    ...
    shareVocabBtn.addEventListener('click', () => window.location.href = 'mailto:sifansong432@qq.com?subject=【分享新题库】&body=请在附件中添加CSV格式的词汇表，一列中文一列英文。');
...
function updateLanguage() {
    ...
    const quizSettingsTitle = document.querySelector('.quiz-settings h2');
    if (quizSettingsTitle) quizSettingsTitle.textContent = t.startQuizTitle;
    
    const settingsOptionsLabel = document.querySelector('.settings-options label');
    if (settingsOptionsLabel) settingsOptionsLabel.textContent = t.questionCountLabel;
    ...
    if (reportErrorBtn) reportErrorBtn.textContent = t.reportErrorBtn;
    if (shareVocabBtn) shareVocabBtn.textContent = t.shareVocabBtn;
    if (suggestImprovementBtn) suggestImprovementBtn.textContent = t.suggestImprovementBtn;
    if (peekAnswerBtn) peekAnswerBtn.textContent = t.peekAnswerBtn;
    if (showAnswerBtn) showAnswerBtn.textContent = t.showAnswerBtn;
    if (correctAnswerBtn) correctAnswerBtn.textContent = t.correctAnswerBtn;
    if (wrongAnswerBtn) wrongAnswerBtn.textContent = t.wrongAnswerBtn;
    if (exitQuizBtn) exitQuizBtn.textContent = t.exitBtn;
    if (backToMainBtn) backToMainBtn.textContent = t.backToMainBtn;
    if (restartQuizBtn) restartQuizBtn.textContent = t.restartQuizBtn;
